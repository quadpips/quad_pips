...
class Go2Visualizer : public ocs2::DummyObserver {
 ...
 public:
   ...
   */
  Go2Visualizer(const kinematic_model_t& kinematicModel, std::vector<std::string> jointNames, std::string baseName,
                const rclcpp::Node::SharedPtr& node, scalar_t maxUpdateFrequency = 1000.0)
      : kinematicModelPtr_(kinematicModel.clone()),
        lastTime_(std::numeric_limits<scalar_t>::lowest()),
        minPublishTimeDifference_(1.0 / maxUpdateFrequency) {
    launchVisualizerNode(node, std::move(jointNames), std::move(baseName));
  };

  ~Go2Visualizer() override = default;
  ...
  void launchVisualizerNode(const rclcpp::Node::SharedPtr& node, std::vector<std::string> jointNames, std::string baseName);

  void publishTrajectory(const std::vector<ocs2::SystemObservation>& system_observation_array, scalar_t speed = 1.0);

  void publishObservation(rclcpp::Time timeStamp, const ocs2::SystemObservation& observation);

  void publishDesiredTrajectory(rclcpp::Time timeStamp, const ocs2::TargetTrajectories& targetTrajectories) const;
 ...
 private:
  void publishCartesianMarkers(rclcpp::Time timeStamp, const contact_flag_t& contactFlags, const feet_array_t<vector3_t>& feetPosition,
                               const feet_array_t<vector3_t>& feetForce) const;
  void publishEndEffectorPoses(rclcpp::Time timeStamp, const feet_array_t<vector3_t>& feetPositions,
                               const feet_array_t<Eigen::Quaternion<scalar_t>>& feetOrientations) const;
  void publishCollisionSpheres(rclcpp::Time timeStamp, const base_coordinate_t& basePose, const joint_coordinate_t& jointAngles) const;

  rclcpp::Node::SharedPtr node_;

  std::unique_ptr<kinematic_model_t> kinematicModelPtr_;

  CustomQuadrupedTfPublisher customQuadrupedTfPublisher_;
