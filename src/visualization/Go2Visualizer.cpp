...
void Go2Visualizer::update(const ocs2::SystemObservation& observation, const ocs2::PrimalSolution& primalSolution,
...
void Go2Visualizer::publishObservation(rclcpp::Time timeStamp, const ocs2::SystemObservation& observation) 
{
  ...
  // Publish
  customQuadrupedTfPublisher_.publish(timeStamp, basePose, qJoints, frameId_);
...
void Go2Visualizer::publishTrajectory(const std::vector<ocs2::SystemObservation>& system_observation_array, double speed) 
{
  for (size_t k = 0; k < system_observation_array.size() - 1; k++) 
  {
    ...
    double publishDuration = ocs2::timedExecutionInSeconds([&]() { publishObservation(node_->get_clock()->now(), system_observation_array[k]); });
    if (frameDuration > publishDuration) 
    {
      rclcpp::Duration duration = rclcpp::Duration::from_seconds(frameDuration - publishDuration);
      rclcpp::sleep_for(std::chrono::nanoseconds(duration.nanoseconds()));
    }
  }
}

void Go2Visualizer::publishCartesianMarkers(rclcpp::Time timeStamp, const contact_flag_t& contactFlags,
...
void Go2Visualizer::publishDesiredTrajectory(rclcpp::Time timeStamp, const ocs2::TargetTrajectories& targetTrajectories) const 
{
...
void Go2Visualizer::publishOptimizedStateTrajectory(rclcpp::Time timeStamp, const scalar_array_t& mpcTimeTrajectory,
...
void Go2Visualizer::publishEndEffectorPoses(rclcpp::Time timeStamp, const feet_array_t<vector3_t>& feetPositions,
...
void Go2Visualizer::publishCollisionSpheres(rclcpp::Time timeStamp, const base_coordinate_t& basePose,
