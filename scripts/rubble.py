...
def make_rubble(args):
    ...
    for y_stone_i in range(0, num_h_stones):
        ...
        for x_stone_i in range(0, num_v_stones):
            ...
            x_noise = x_noise_lims[0] + np.random.random_sample() * (x_noise_lims[1] - x_noise_lims[0]) # 0.0 # 
            
            y_noise_lims = [-0.03, 0.03] # m
            y_noise = y_noise_lims[0] + np.random.random_sample() * (y_noise_lims[1] - y_noise_lims[0]) # 0.0 # 
            ...
            roll_noise = roll_noise_lims[0] + np.random.random_sample() * (roll_noise_lims[1] - roll_noise_lims[0]) # 0.0 # 
            
            pitch_noise_lims = [-0.25, 0.25] # radians
            pitch_noise = pitch_noise_lims[0] + np.random.random_sample() * (pitch_noise_lims[1] - pitch_noise_lims[0]) # 0.0 # 

            yaw_noise_lims = [-0.25, 0.25] # radians
            yaw_noise = yaw_noise_lims[0] + np.random.random_sample() * (yaw_noise_lims[1] - yaw_noise_lims[0]) # 0.0 # 
            ...
            stone_center_pose_env_frame[2, 0] += (z_spacing if x_stone_i < (num_v_stones / 2) else -z_spacing)
