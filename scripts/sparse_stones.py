...
def make_sparse_stones(args):
    print("[make_sparse_stones()]")
    ...
    stone_center_pose_env_frame = np.array([[-(start_platform_width / 2.0) - 1.5], #
                                            [0.0],
    ...
    ## flat
    stone_center_pose_env_frame = np.array([[-1.5 + (stone_length / 2.0)], # 
                                            [0.0],
                                            [-(stone_height / 2.0)]])
    ...
    counter = 0
    for y_stone_i in range(1, num_h_stones + 1):
        counter += 1
        for x_stone_i in range(1, num_v_stones + 1):
            ...
            if (sample == 1 or sample == 3):  # (raised)
        ...
        stone_center_pose_env_frame[0, 0] = -1.5 + (stone_length / 2.0)
        stone_center_pose_env_frame[1, 0] += y_spacing
        stone_center_pose_env_frame[2, 0] =  -(stone_height / 2.0)
